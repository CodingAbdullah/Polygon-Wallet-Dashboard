...
export default function MetricsNavbar() {
    ...
    const { data: arbData, error: ethError, isLoading: arbLoading } = useSWR<{ tokenPrice: NavbarArbitrumDataType }>('/api/arbitrum-price-data', GenericFetcher, { refreshInterval: 50000 });
    ...
    else if (arbData && gasData) {
        const { arbitrum }  = arbData.tokenPrice;
                ...
                <div className="container mx-auto flex justify-between items-center">
                    <div className="flex space-x-4 items-center">
                        <div className="flex items-center space-x-2">
                            ...
                            <span>Arb Price: <span>${ Number(arbitrum?.usd).toFixed(2) }</span></span>
                        <span>
                            24-Hr % Chg: 
                            <span className={ Number(arbitrum?.usd_24h_change) >= 0 ? 'text-green-500' : 'text-red-500' }>
                                { Number(arbitrum?.usd_24h_change) > 0 ? ' +' : ' ' }
                                { Number(arbitrum?.usd_24h_change).toFixed(2) }%
                            </span>
                        </span>
